...
def search_novels(app):
    ...
    # order by the number of novels found then by SequenceMatcher ratio
    processed.sort(key=lambda x: (-len(x.novels), -difflib.SequenceMatcher(None, x.title, app.user_input).ratio()))

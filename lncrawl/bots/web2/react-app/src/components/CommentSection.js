...
function CommentSection({ comments, setReplyTo, setCommenting }) {
    ...
    function sortComments(type, object) {
        if (type === 'date') {
            ...
            return Object.entries(object).sort((a, b) => Object.keys(b[1].replies).length - Object.keys(a[1].replies).length)
        } else if (type === 'reactions') {
            return Object.entries(object).sort((a, b) => (b[1].likes + b[1].dislikes + numberOfReplies(b[1])) - (a[1].likes + a[1].dislikes + numberOfReplies(a[1])))
        } else {
